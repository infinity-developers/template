...
class Presence(commands.GroupCog):
    ...
    @discord.discord.app_commands.command(name="setstatus", description="Set the bot's status and activity")
    ...
    async def setstatus(
        ...
        status_text:str
            ...
            discord_status = discord.Status[status.value]
            
            if activity_type.value == "playing":
                discord_activity = discord.Game(name=activity,status=status_text)
            elif activity_type.value == "listening":
                discord_activity = discord.Activity(type=discord.ActivityType.listening, name=activity, status=status_text)
            elif activity_type.value == "watching":
                discord_activity = discord.Activity(type=discord.ActivityType.watching, name=activity, status=status_text)
            else:
